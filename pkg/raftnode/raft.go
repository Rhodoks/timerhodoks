...
func (rc *RaftNode) GetState() raft.StateType { //返回当前节点的状态（Leader, Folloer...)，只有Leader节点可以充当Coordinator进行任务分配的协调
	if rc.node == nil {
		return raft.StateCandidate
	}
	return rc.node.Status().SoftState.RaftState
}

var defaultSnapshotCount uint64 = 20000 //每20000条log进行一次快照
...
func NewRaftNode(id int, peers []string, join bool, getSnapshot func() ([]byte, error), proposeC <-chan string,
	confChangeC <-chan raftpb.ConfChange) (<-chan *Commit, <-chan error, <-chan *snap.Snapshotter, *RaftNode) {
